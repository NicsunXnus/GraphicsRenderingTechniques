...
// This value tells when to terminate the progressive refinement radiosity
// computation. It sets the maximum number of iterations.
...
// Window size.
static int winWidthHeight =
    600;  // Window width & height in pixels. Must be even number.
...
static void ReadColorBuffer(GLubyte *buf, bool frontBuffer, int x, int y,
...
// If frontBuffer == true, it reads the front buffer, otherwise, the back
// buffer. The read color buffer region is stored in the 1-D array buf[], which
// must be pre-allocated enough memory space.
{
...
// Used for rendering the quads for the hemicube.
{
  ...
  if (dlist == 0)
    ShowFatalError(__FILE__, __LINE__, "Cannot create display list");
...
static int FindShooterQuadWithHighestUnshotPower(const QM_Model *m) {
  ...
  for (int q = 0; q < m->totalShooters; q++) {
    ...
    if (RGBunshotPower > maxUnshotPower) {
      maxUnshotPower = RGBunshotPower;
      s = q;
    }
  }
  return s;
...
static float TriangleArea(const float v1[3], const float v2[3],
                          const float v3[3])
...
// Compute the width of the hemicube such that it is within the boundary of the
// quad.
{
  const float SQRT_2 = 1.414214f;

  float c01 =
      TriangleArea(shooterQuad->centroid, shooterQuad->v[0], shooterQuad->v[1]);
  float c12 =
      TriangleArea(shooterQuad->centroid, shooterQuad->v[1], shooterQuad->v[2]);
  float c23 =
      TriangleArea(shooterQuad->centroid, shooterQuad->v[2], shooterQuad->v[3]);
  float c30 =
      TriangleArea(shooterQuad->centroid, shooterQuad->v[3], shooterQuad->v[0]);
...
static void PreComputeTopFaceDeltaFormFactors(float deltaFormFactors[],
                                              int numPixelsOnWidth)
...
static void PreComputeSideFaceDeltaFormFactors(float deltaFormFactors[],
                                               int numPixelsOnWidth)
...
// Note that numPixelsOnWidth must be a even number.
{
  ...
  **********************************************************/
  double dp = 2.0 / numPixelsOnWidth;  // Width of a pixel.
  double dh = dp;                      // Height of pixel
  double dA = Sqr(dp);                 // Area of a pixel.
  int numPixelsOnHeight = numPixelsOnWidth / 2;
  for (int pz = 0; pz < numPixelsOnHeight; pz++) {
    double z = (pz + 0.5) * dh;  

    for (int py = 0; py < numPixelsOnWidth; py++) {
      double y = -1.0 + (py + 0.5) * dp;
      double dFq = (z * dA) / (M_PI * Sqr(z * z + y * y + 1.0));
      deltaFormFactors[pz * numPixelsOnWidth + py] = (float)dFq;
    }
  }
}

static void SetupHemicubeTopView(const QM_ShooterQuad *shooterQuad,
...
// Need to set up the viewport, projection and view transfromation.
{
  ...
  glLoadIdentity();
  gluLookAt(shooterQuad->centroid[0], shooterQuad->centroid[1],
            shooterQuad->centroid[2], lookAt[0], lookAt[1], lookAt[2],
            upVector[0], upVector[1], upVector[2]);
}

static void SetupHemicubeSideView(int face, const QM_ShooterQuad *shooterQuad,
                                  float nearPlane, float farPlane)
// Set up a view for one of the four side faces of a hemicube.
// The input parameter face is a number from 1 to 4, that indicates which face
// to set up for. Need to set up the viewport, projection and view
// transfromation.
{
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  ...
  **********************************************************/
  ...
  gluLookAt(shooterQuad->centroid[0], shooterQuad->centroid[1],
            shooterQuad->centroid[2], lookAt[0], lookAt[1], lookAt[2],
            shooterQuad->normal[0], shooterQuad->normal[1],
            shooterQuad->normal[2]);
}

static void UpdateRadiosities(const QM_Model *m, const float shotPower[3],
                              const GLubyte colorBuf[],
                              const float deltaFormFactors[], int width,
                              int height)
// Use the color buffer (item buffer) to update the radiosities of the gatherer
// quads, and update the unshot power of their parent shooter quads.
{
...
static void ComputeRadiosity(void) {
  ...
  GLubyte *colorBuf = (GLubyte *)CheckedMalloc(sizeof(GLubyte) * 3 *
  ...
  for (int iterationCount = 0; iterationCount < maxIterations;
    ...
    float unshotPower[3] = {shooterQuad->unshotPower[0],
                            shooterQuad->unshotPower[1],
                            shooterQuad->unshotPower[2]};
    printf("Iteration %d\n", iterationCount);

    // After shooting power, the shooter quad's unshot power becomes zero.
    shooterQuad->unshotPower[0] = shooterQuad->unshotPower[1] =
        shooterQuad->unshotPower[2] = 0.0f;
    ...
    // Top face.
    SetupHemicubeTopView(shooterQuad, hemicubeWidth / 2.0f,
                         2.0f * model.radius);
    glCallList(gathererQuadsDList);
    glFinish();
    ReadColorBuffer(colorBuf, true, 0, 0, winWidthHeight, winWidthHeight);
    UpdateRadiosities(&model, unshotPower, colorBuf, topDeltaFormFactors,
                      winWidthHeight, winWidthHeight);
    printf("Iteration %d\n", iterationCount);

    // Side faces.
    for (int face = 1; face <= 4; face++) {
      SetupHemicubeSideView(face, shooterQuad, hemicubeWidth / 2.0f,
                            2.0f * model.radius);
      glCallList(gathererQuadsDList);
      glFinish();
      ReadColorBuffer(colorBuf, true, 0, 0, winWidthHeight, winWidthHeight / 2);
      UpdateRadiosities(&model, unshotPower, colorBuf, sideDeltaFormFactors,
                        winWidthHeight, winWidthHeight / 2);
    }
  }
...
static void InitRadiosityComputation(void) {
  ...
  topDeltaFormFactors =
      (float *)CheckedMalloc(sizeof(float) * winWidthHeight * winWidthHeight);
  sideDeltaFormFactors = (float *)CheckedMalloc(sizeof(float) * winWidthHeight *
                                                winWidthHeight / 2);
  ...
  for (int s = 0; s < model.totalShooters; s++) {
    QM_ShooterQuad *shooterQuad = model.shooters[s];
    shooterQuad->unshotPower[0] =
        shooterQuad->area * shooterQuad->surface->emission[0];
    shooterQuad->unshotPower[1] =
        shooterQuad->area * shooterQuad->surface->emission[1];
    shooterQuad->unshotPower[2] =
        shooterQuad->area * shooterQuad->surface->emission[2];
  }
...
int main(int argc, char **argv) {
  ...
  glutInitWindowSize(
      winWidthHeight,
      winWidthHeight);  // Window must be square and size is fixed.
  ...
  if (winWidth != winHeight || winWidth % 2 != 0)
    ShowFatalError(__FILE__, __LINE__,
                   "Window size is not square or its width is not even");
  winWidthHeight = winWidth;

  printf("\nIMPORTANT:\n");
  printf("Do not minimize, move, resize, or cover the drawing window.\n\n");
  printf(
      "Press ENTER in this console window to start the radiosity "
      "computation.\n");
  char ch;
  scanf("%c", &ch);
