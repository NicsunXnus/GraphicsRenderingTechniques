...
void DrawSceneWithProjection()
{
    ...
    // * Calculate the shadowmap coordinates of this fragment.
    vec4 shadCoord = ShadowMatrix * vec4(wcPosition, 1.0f);
    //Perspective divide
    shadCoord = vec4((shadCoord.xyz / shadCoord.w), 1.0f);
    shadCoord.z = shadCoord.z - 0.02; //subtract tolerance value from z to reduce shadow acne
    // The sum of the comparisons with nearby texels
    float sum = 0.0;
    // Sum contributions from texels around ShadowCoord
    sum += textureProjOffset(ShadowMap, shadCoord, ivec2(-1,-1));
    sum += textureProjOffset(ShadowMap, shadCoord, ivec2(-1,1));
    sum += textureProjOffset(ShadowMap, shadCoord, ivec2(1,1));
    sum += textureProjOffset(ShadowMap, shadCoord, ivec2(1,-1));
    float shadowFact = sum * 0.25;

    //The above PCF calculation implicitly sets fragments outside the FOV of the projector to
    // be in shadow  (shadowFact = 0) as when a texel falls outside the valid range, the corresponding texture fetch returns 0.

    // * Compute lighting using projector's position as the light source
    //   position. Use Phong lighting model.
    vec3 ambientColor = LightAmbient * MatlAmbient;// * The light's ambient coefficient is LightAmbient (uniform variable).
    float N_dot_L, R_dot_V_pow_n;
    PhongLighting(N_dot_L, R_dot_V_pow_n);
    // * Use the color retrieved from ProjectorImage as both the
    //   diffuse coefficient and specular coefficient of the light source
    vec3 projColor = texture(ProjectorImage, shadCoord.st).rgb;
