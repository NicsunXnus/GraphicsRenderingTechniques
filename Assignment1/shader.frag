...
void drawBrickCube()
{
    if (gl_FrontFacing) {
        ...
        vec3 B,T;
        vec2 tex2dCoord = vec2(v2fTexCoord);
        compute_tangent_vectors(necNormal, ecPosition, tex2dCoord, T, B);
        // * Read and decode tangent-space perturbation vector from normal map.
        //read from map
        vec3 tanPerturbedNormal = texture( BrickNormalMap, tex2dCoord ).rgb; 
        //decode
        ...
        vec3 color = texture(BrickDiffuseMap, tex2dCoord).rgb;
        //PIE : Ia * Ka + Ip * Kd * (N . L) + Ip * Ks(R . V)^n
        color = LightAmbient.rgb * color + LightDiffuse.rgb * N_dot_L * color + LightSpecular.rgb * BrickSpecularMaterial * spec;
...
void drawWoodenCube()
{
    if (gl_FrontFacing) {
        ...
        float diff = MirrorRadius * MirrorRadius - sqrDist; 
        //If sqrtDist > MirrorRadius * MirrorRadius, it is outside of the mirror.
        //Therefore, diff < 0 -> wood, else -> mirror
        ...
        {
            //    -- Read from wood texture map.
            vec3 woodTexColor = texture( WoodDiffuseMap, tex2dCoord ).rgb;
        ...
        else
        {
            // -- Construct tangent-space perturbation vector for a
            //       hemispherical bump.
            vec3 tanPerturbedNormal = normalize( vec3(-p, sqrt(diff) ) );
