...
void InitScene()
{
    ...
    float A = PI/4.;
    float L = 3.0;
    float T = 2.*PI*sqrt(L/g);
    float t = iTime;
    float offset = 1.0;
    float verticalPos = L + offset;
    float theta = A*cos( (2.*PI/T) * t );
    
    //Longest swinging pendulum
    Sphere[0].center = vec3(L* sin(theta) + 1.0 , -L*cos(theta) + verticalPos ,2.);
    Sphere[0].radius = 0.2;
    Sphere[0].materialID = 2;

    // Next shortest
    L = L - 0.5;
    verticalPos = L + offset;
    T = 2.*PI*sqrt(L/g);
    theta = A*cos( (2.*PI/T) * (t - 0.01) );
    Sphere[1].center = vec3(L* sin(theta) + 1.0 , -L*cos(theta) + verticalPos ,1.5);
    Sphere[1].radius = 0.2;
    ...
    verticalPos = L + offset;
    T = 2.*PI*sqrt(L/g);
    theta = A*cos( (2.*PI/T) * (t - 0.02) );
    Sphere[2].center = vec3(L* sin(theta) + 1.0 , -L*cos(theta) + verticalPos ,.5);
    Sphere[2].radius = 0.2;
    ...
    verticalPos = L + offset;
    T = 2.*PI*sqrt(L/g);
    theta = A*cos( (2.*PI/T) * (t - 0.03) );
    Sphere[3].center = vec3(L* sin(theta) + 1.0 , -L*cos(theta) + verticalPos ,.0);
    Sphere[3].radius = 0.2;
    ...
    verticalPos = L + offset;
    T = 2.*PI*sqrt(L/g);
    theta = A*cos( (2.*PI/T) * (t - 0.04) );
    Sphere[4].center = vec3(L* sin(theta) + 1.0 , -L*cos(theta) + verticalPos ,-.5);
    Sphere[4].radius = 0.2;
    ...
    // Red material.
    Material[2].k_d = vec3( 1.0, 0., 0. );
    Material[2].k_a = 0.2 * Material[1].k_d;
    Material[2].k_r = 2.0 * Material[1].k_d;
    Material[2].k_rg = 0.5 * Material[1].k_r;
    Material[2].n = 64.0;
    
    // Blue material.
    Material[3].k_d = vec3( 0.0, 0., 1. );
    ...
    // Purple material.
    Material[5].k_d = vec3( .0, 1., 1. );
    Material[5].k_a = 0.2 * Material[1].k_d;
    Material[5].k_r = 2.0 * Material[1].k_d;
    ...
    // Light 0 spinning in a circle
    Light[0].position = vec3( 3.0 * cos(t), 8.0 , 3.0 * sin(t));
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 I_result_global = vec3(0.0f);
    for (int i = 0; i < SPP; i++) {
        // Generate random offset for fragCoord for anti aliasing, 0.5 randomly chosen for both x and y coords perturbing
        // since from experiments, using values other than 0.5, and also different valus for x and y did not change the
        // visuals much.
        ...
        // Generate random offset for cam pos for DOF effect, same as before 0.5 was randonmly chosen
        pRay.o = cam_pos + aperture_width * (cam_x_axis * (rand() - 0.5)  + cam_y_axis * (rand() - 0.5)  );
        ...
        I_result_global += I_result;
    }

    fragColor = vec4( I_result_global / float(SPP) , 1.0 );
}
